...
export default function PressReleases() {
  ...
  return (
    <>
      <SEOHead title="Press Releases" description="Official press releases from the Controller & Accountant-General's Department." path="/news/press-releases" />
      ...
      >
        ...
        <div className="container max-w-6xl relative z-10">
          <Breadcrumbs items={[{ label: "News", href: "/news" }, { label: "Press Releases" }]} />
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-3xl md:text-5xl font-heading font-bold mb-3">
            Press Releases
          </motion.h1>
          <p className="text-white/80 max-w-xl">Official statements and press releases from CAGD.</p>
        </div>
      </section>
      ...
      <section className="py-10 bg-background">
        <div className="container max-w-6xl">
          <div className="relative max-w-xs mb-8">
            ...
            <Input placeholder="Search press releases..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="pl-10" />
          </div>
                    ...
                    <motion.article key={article.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }} className="bg-card border border-border rounded-lg overflow-hidden group hover:border-primary/30 transition-colors">
                      ...
                      <div className="p-4">
                        <div className="flex items-center gap-2 mb-2">
                          <Badge variant="secondary" className="text-xs px-2 py-0">Press Release</Badge>
          ...
          {!isLoading && filtered.length === 0 && (
            <div className="text-center py-16 text-muted-foreground">No press releases found.</div>
          )}

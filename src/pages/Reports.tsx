...
function GlowDownloadButton({ fileUrl, reportId }: { fileUrl: string; reportId: string }) {
  const handleDownload = async () => {
    ...
    const resolvedUrl = fileUrl.startsWith("/new-site") ? fileUrl : `/new-site${fileUrl}`;
...
function StatsBar({ total, categories, withFiles }: { total: number; categories: number; withFiles: number }) {
  const items = [
    ...
    { label: "Downloadable", value: withFiles, icon: TrendingUp },
  ];
  return (
    <motion.div
    ...
    <>
      ...
      >
        ...
        <div className="container max-w-5xl relative z-10">
          <Breadcrumbs items={[{ label: "Reports & Documents" }]} />
          <motion.h1
          ...
          >
            Access official financial reports, quarterly accounts, payroll data, and publications from CAGD.
          </motion.p>
        </div>
      </section>

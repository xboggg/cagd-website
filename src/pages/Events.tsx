...
function CountdownUnit({ value, label }: { value: number; label: string }) {
  return (
    <div className="text-center">
      <div className="text-2xl md:text-3xl font-heading font-bold text-primary tabular-nums">{value}</div>
          ...
          <div className="flex-1 p-6 lg:p-8 flex flex-col justify-between gap-6">
            ...
            {countdown && (
              <div className="flex items-center gap-0">
                <div className="flex items-center gap-5 bg-background/80 backdrop-blur-sm border border-border rounded-xl px-5 py-3">
        ...
        <div className="flex-1 min-w-0 py-3 px-4 flex items-center gap-4">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-0.5">
              {event.featured && !isPast && <Badge className="bg-secondary/90 text-secondary-foreground text-[10px] px-1.5 py-0 h-4">Featured</Badge>}
              <CategoryBadge category={event.category} />
          ...
          {showCountdown && countdown ? (
            ...
            <Badge variant="secondary" className="text-[10px] flex-shrink-0 hidden sm:inline-flex">Completed</Badge>
          ) : null}
...
function StatsBar({ upcoming, past, total }: { upcoming: number; past: number; total: number }) {
  return (
    <div className="grid grid-cols-3 gap-4 mb-8">
      {[
        { value: upcoming, label: "Upcoming", color: "text-primary" },
        { value: past, label: "Past Events", color: "text-muted-foreground" },
                  ...
                  {(["upcoming", "past"] as const).map((t) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className={`ml-0.5 ${tab === t ? "text-primary-foreground/80" : "text-muted-foreground/60"}`}>({count})</span>
                      </button>
                    );
                  })}
                </div>
                  ...
                  >
                    {filtered.length === 0 ? (
                      <div className="text-center py-16 text-muted-foreground">
                        ...
                        <p className="text-sm">
                          {search ? `No ${tab} events matching "${search}".` : `No ${tab} events at this time.`}
                        </p>
                      </div>
                    ) : (

...
import { Search, Calendar, ArrowRight, ChevronLeft, ChevronRight, Loader2, FileText, Download } from "lucide-react";
...
export default function Digest() {
  ...
  return (
    <>
      <SEOHead title="CAGD Digest" description="Weekly digest of activities and updates from the Controller & Accountant-General's Department." path="/news/digest" />
      ...
      >
        ...
        <div className="container max-w-6xl relative z-10">
          <Breadcrumbs items={[{ label: "News", href: "/news" }, { label: "CAGD Digest" }]} />
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-3xl md:text-5xl font-heading font-bold mb-3">
            CAGD Digest
          </motion.h1>
          <p className="text-white/80 max-w-xl">Weekly digest of activities and updates from the department.</p>
        </div>
      </section>
      ...
      <section className="py-10 bg-background">
        <div className="container max-w-6xl">
          <div className="relative max-w-xs mb-8">
            ...
            <Input placeholder="Search digest..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="pl-10" />
          </div>
            ...
            <AnimatePresence mode="wait">
              <motion.div key={`${page}-${search}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {paginated.map((article, idx) => {
                  ...
                  const fileUrl = (article as any).file_url as string | null;
                  return (
                      ...
                      <Link to={articleUrl}>
                        <div className="h-72 bg-muted overflow-hidden">
                          {imageUrl ? (
                            ...
                            <DigestCover title={article.title} date={article.publish_date ?? undefined} />
                          )}
                        </div>
                      </Link>
                      <div className="p-4">
                        <div className="flex items-center gap-2 mb-2">
                          <Badge variant="secondary" className="text-xs px-2 py-0">Digest</Badge>
                        ...
                        <h3 className="font-heading font-semibold text-foreground text-sm leading-tight mb-2 line-clamp-2">
                          <Link to={articleUrl} className="hover:text-primary transition-colors">{article.title}</Link>
                        </h3>
          ...
          {!isLoading && filtered.length === 0 && (
            <div className="text-center py-16 text-muted-foreground">
              ...
              <p>No digest issues published yet. Check back soon!</p>
            </div>
          )}

...
import { Search, Calendar, ArrowRight, ChevronLeft, ChevronRight, Loader2, Download } from "lucide-react";
...
export default function TreasuryNews() {
  ...
  const { data: articles = [], isLoading } = useQuery({
    queryKey: ["treasury-news"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("cagd_news")
        .select("*")
        .eq("status", "published")
        .eq("category", "Treasury News")
  ...
  return (
    <>
      <SEOHead title="Treasury News" description="News and updates from the Treasury division of the Controller & Accountant-General's Department." path="/news/treasury" />
      ...
      >
        ...
        <div className="container max-w-6xl relative z-10">
          <Breadcrumbs items={[{ label: "News", href: "/news" }, { label: "Treasury News" }]} />
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-3xl md:text-5xl font-heading font-bold mb-3">
            Treasury News
          </motion.h1>
          <p className="text-white/80 max-w-xl">News and updates from the Treasury division.</p>
        </div>
      </section>
      ...
      <section className="py-10 bg-background">
        <div className="container max-w-6xl">
          <div className="relative max-w-xs mb-8">
            ...
            <Input placeholder="Search treasury news..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="pl-10" />
          </div>
                    ...
                    <motion.article key={article.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }} className="bg-card border border-border rounded-lg overflow-hidden group hover:border-primary/30 transition-colors">
                      ...
                      <div className="p-4">
                        <div className="flex items-center gap-2 mb-2">
                          <Badge variant="secondary" className="text-xs px-2 py-0">Treasury News</Badge>
          ...
          {!isLoading && filtered.length === 0 && (
            <div className="text-center py-16 text-muted-foreground">No treasury news articles found.</div>
          )}

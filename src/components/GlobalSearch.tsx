          ...
          >
            <div className="bg-card sm:rounded-2xl shadow-2xl border-b sm:border border-border overflow-hidden mx-0 sm:mx-0">
              ...
              <div className="flex items-center gap-3 px-4 border-b border-border">
                ...
                {!query && (
                  <kbd className="hidden sm:inline-flex rounded bg-muted px-1.5 py-0.5 text-[10px] text-muted-foreground font-mono">ESC</kbd>
                )}
              </div>
              ...
              <div className="max-h-[calc(100dvh-56px)] sm:max-h-72 overflow-y-auto overscroll-contain">
                ...
                {!loading && query.length >= 2 && results.length === 0 && (
                  <p className="text-center text-muted-foreground py-8 text-sm">No results for &ldquo;{query}&rdquo;</p>
                )}
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[11px] text-muted-foreground">
                            {result.type === "news" ? "News" : "Report"}{result.category ? ` \u2022 ${result.category}` : ""}
                          </p>
                        </div>
                      </button>
                    ))}
                ...
                {!loading && query.length < 2 && (
                  <p className="text-center text-muted-foreground/50 py-6 text-xs">Type to search</p>
                )}
              </div>
            </div>
          </motion.div>
